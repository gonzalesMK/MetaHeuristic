...
class BinaryBlackHole(_BaseMetaHeuristic):
    ...
    def __init__(self, classifier=None, number_gen=10, size_pop=40, verbose=False,
        ...
        self.toolbox.register("star", tools.initIterate,
                              BaseMask, self.toolbox.attribute)
        self.toolbox.register("galaxy", tools.initRepeat,
                              list, self.toolbox.star)
    ...
    def fit(self, X=None, y=None, normalize=False, **arg):
        ...
        **arg : parameters
        ...
        self.best_mask_ = np.asarray(best[0][:], dtype=np.bool)
    ...
    def _updateStar(self, star, blackhole):
        if self._dist(star, blackhole) < blackhole.radius:
            ...
            star[:] = [1 if abs(np.tanh(star[x] + self._random_object.uniform(0, 1) * (blackhole[x] - star[x])))
                       > self._random_object.uniform(0, 1) else 0 for x in range(0, self.n_features_)]
