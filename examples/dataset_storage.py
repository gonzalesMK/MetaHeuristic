...
COLUNAS = ["experiment_id","algorithm","dataset","gen","acummulativeTime","hallOfFame","paretoFront",
...
if( not exists):
    cursor.execute("CREATE TABLE {}( ".format(db_name) + """
            ...
            acummulativeTime DOUBLE NOT NULL,
            hallOfFame array NOT NULL,
            paretoFront array NOT NULL,
...
for name in meta_names:
    ...
    for filename in filenames:
        ...
        values.append( home + name + "/" + filename)
        ...
        gen_n_individuals = np.reshape( [log['gen'] for log in logbook], (-1,1))
        gen_time = np.reshape( [ "{:.0f}".format(np.floor( (i['time']-logbook[0]['time'])/60)) for i in logbook], (-1,1))
        hofs = [ log['hallOfFame'] for log in logbook]
        paretos =  [ ( log['paretoFront'], None) for log in logbook]
        gen_25_matthews = np.reshape( logbook.chapters["fitness"].select("25_percentile"), (-1,1))
        gen_50_matthews = np.reshape( logbook.chapters["fitness"].select("50_percentile"), (-1,1))
        gen_75_matthews = np.reshape( logbook.chapters["fitness"].select("75_percentile"), (-1,1))
        gen_min_matthews= np.reshape( logbook.chapters["fitness"].select("min"), (-1,1))
        gen_avg_matthews= np.reshape( logbook.chapters["fitness"].select("avg"), (-1,1))
        gen_std_matthews= np.reshape( logbook.chapters["fitness"].select("std"), (-1,1))
        gen_max_matthews= np.reshape( logbook.chapters["fitness"].select("max"), (-1,1))
        gen_25_feature  = np.reshape(logbook.chapters["size"].select("25_percentile"), (-1,1))
        gen_50_feature  = np.reshape(logbook.chapters["size"].select("50_percentile"), (-1,1))
        gen_75_feature  = np.reshape(logbook.chapters["size"].select("75_percentile"), (-1,1))
        gen_min_feature = np.reshape(logbook.chapters["size"].select("min"), (-1,1))
        gen_avg_feature = np.reshape(logbook.chapters["size"].select("avg"), (-1,1))
        gen_std_feature = np.reshape(logbook.chapters["size"].select("std"), (-1,1))
        gen_max_feature = np.reshape(logbook.chapters["size"].select("max"), (-1,1))
        ...
        attrList = np.concatenate((
            ...
            np.reshape([filename]*len(gen_n_individuals), (-1,1)),
            gen_n_individuals,
            gen_time,
        ...
        [attrList[i].insert(5, np.array(hofs[i])) for i in range(len(hofs))]
        [attrList[i].insert(6, np.array(paretos[i])) for i in range(len(paretos))]
